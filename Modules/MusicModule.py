...
ytdl_format_options = {
    ...
    'paths': {'home': "Songs", 'thumbnail': 'Images/Videos'}
}
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def from_url(cls, url, *, loop=None, stream=False):
        ...
        if 'entries' in data:
            data = data['entries'][0]
        filename = os.path.join("songs", data['title']) if stream else ytdl.prepare_filename(data)
        return filename
    @classmethod
...
class Player():
    ...
    async def waitforend(self, interaction, queue):
        ...
        if queue[0]["userfile"]:
            ...
            if queue[0]["coverart"] != "Songs/Images/generic-thumb.png":
...
async def PlayCommand(interaction: discord.Interaction, query: str, client: discord.Client):
    ...
    if result == None:
        ...
        if os.path.exists(f"Songs/Images/Videos/{os.path.splitext(os.path.basename(song['filename']))[0]}.webp"):
            song["coverart"] = f"Songs/Images/Videos/{os.path.splitext(os.path.basename(song['filename']))[0]}.webp"
        else:
...
async def PlayFileCommand(interaction: discord.Interaction, file: discord.Attachment, client):
    ...
    file_name = f"Songs/{str(file).split('/')[6].split('?')[0]}"
